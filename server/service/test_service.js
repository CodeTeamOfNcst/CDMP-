import { Apply,Device,DeviceType,Message,MessageKlass,Rule,User,UserKlass, } from '../dbconfig/dbinit'
...
module.exports.findOrCreateData = async ( ctx, next ) => {
    ...
    let successMessageType = await MessageKlass.findOrCreate({ where: { name: '成功' } })[0]
    let commonMessageType = await MessageKlass.findOrCreate({ where: { name: '普通消息'} })[0]
    let failMessageType = await MessageKlass.findOrCreate({ where: { name: '失败消息'} })[0]

    let successMessage = await Message.findOrCreate({ where: { content: '这是一条成功的消息', isPublished: false, isRead: false, deviceType: successMessageType} })[0]
    let commonMessage = await Message.findOrCreate({ where: { content: '这是一条普通的消息', isPublished: false, isRead: false, deviceType: commonMessageType} })[0]
    let failMessage = await Message.findOrCreate({ where: { content: '这是一条失败的消息', isPublished: false, isRead: false, deviceType: failMessageType} })[0]

    let computerDeviceType = await DeviceType.findOrCreate({ where: { name: '计算机系' } })[0]
    let chemicalDeviceType = await DeviceType.findOrCreate({ where: { name: '化学系' } })[0]
    let metallurgicalDeviceType = await DeviceType.findOrCreate({ where: { name: '冶金系' } })[0]

    let device1 = await Device.findOrCreate({ where: { name: '计算机系的仪器', description: '这是计算机系的仪器', deviceType: computerDeviceType, imgFilePath: '/upload/device/device1' } })[0]
    let device2 = await Device.findOrCreate({ where: { name: '化工系的仪器', description: '这是化工系的仪器', deviceType: chemicalDeviceType, imgFilePath: '/upload/device/device2' } })[0]
    let device3 = await Device.findOrCreate({ where: { name: '冶金系的仪器', description: '这是冶金系的仪器', deviceType: metallurgicalDeviceType, imgFilePath: '/upload/device/device3' } })[0]

    let apply1 = await Apply.findOrCreate({ where: { vioReason: 'user1 想要用计算机系的仪器', applyer: user1, applyDevice: device1 } })[0]
    let apply2 = await Apply.findOrCreate({ where: { vioReason: 'user2 想要用化学系的仪器', applyer: user2, applyDevice: device2 } })[0]
    let apply3 = await Apply.findOrCreate({ where: { vioReason: 'user3 想要用冶金系的仪器', applyer: user3, applyDevice: device2 } })[0]
...
exports.getOrCreateData = async ( ctx, next ) => {
    ...
    let successMessageType = await getOrCreate(MessageKlass, { name: '成功' })
    let commonMessageType = await getOrCreate(MessageKlass, { name: '普通消息' })
    let failMessageType = await getOrCreate(MessageKlass, { name: '失败消息' })

    let successMessage = await getOrCreate(Message, { content: '这是一条成功的消息', isPublished: false, isRead: false, deviceType: successMessageType })
    let commonMessage = await getOrCreate(Message, { content: '这是一条普通的消息', isPublished: false, isRead: false, deviceType: commonMessageType })
    let failMessage = await getOrCreate(Message, { content: '这是一条失败的消息', isPublished: false, isRead: false, deviceType: failMessageType })

    let computerDeviceType = await getOrCreate(DeviceType, { name: '计算机系' })
    let chemicalDeviceType = await getOrCreate(DeviceType, { name: '化学系' })
    let metallurgicalDeviceType = await getOrCreate(DeviceType, { name: '冶金系' })

    let device1 = await getOrCreate(Device, { name: '计算机系的仪器', description: '这是计算机系的仪器', deviceType: computerDeviceType, imgFilePath: '/upload/device/device1' })
    let device2 = await getOrCreate(Device, { name: '化工系的仪器', description: '这是化工系的仪器', deviceType: chemicalDeviceType, imgFilePath: '/upload/device/device2' })
    let device3 = await getOrCreate(Device, { name: '冶金系的仪器', description: '这是冶金系的仪器', deviceType: metallurgicalDeviceType, imgFilePath: '/upload/device/device3' })

    let apply1 = await getOrCreate(Apply, { vioReason: 'user1 想要用计算机系的仪器', applyer: user1, applyDevice: device1 })
    let apply2 = await getOrCreate(Apply, { vioReason: 'user2 想要用化学系的仪器', applyer: user2, applyDevice: device2 })
    let apply3 = await getOrCreate(Apply, { vioReason: 'user3 想要用冶金系的仪器', applyer: user3, applyDevice: device2 })
...
exports.createData = async ( ctx, next ) => {
    try{
        ...
        let user5 = await User.create( { account: 'chenwenwen', password: '123456', name: '陈雯雯', email: 'chenwenwen@qq.com' } );
        await user5.setUserType(badGay);
        let user6 = await User.create( { account: 'chenmotong', password: '123456', name: '陈墨瞳', email: 'chenmotong@qq.com' } );
        await user6.setUserType(commonUser);
        let user7 = await User.create( { account: 'kaisa', password: '123456', name: '凯撒', email: 'kaisa@qq.com' } );
        await user7.setUserType(badGay);
        let user8 = await User.create( { account: 'chuzihang', password: '123456', name: '楚子航', email: 'chuzihang@qq.com' } );
        await user8.setUserType(commonUser);
        let user9 = await User.create( { account: 'lumingze', password: '123456', name: '路明泽', email: 'lumingze@qq.com' } );
        await user9.setUserType(badGay);
        let user10 = await User.create( { account: 'liuzhenhan', password: '123456', name: '刘震撼', email: 'liuzhenhan@qq.com' } );
        await user10.setUserType(commonUser);
        let user11 = await User.create( { account: 'hailun', password: '123456', name: '海伦', email: 'hailun@qq.com' } );
        await user11.setUserType(badGay);
        let user12 = await User.create( { account: 'halibote', password: '123456', name: '哈利珀特', email: 'halibote@qq.com' } );
        await user12.setUserType(commonUser);
        let user13 = await User.create( { account: 'ningyu', password: '123456', name: '凝玉', email: 'ningyu@qq.com' } );
        ...
        let successMessage1 = await Message.create( { content: '这是一条活久见成功的消息', isPublished: false, isRead: false } );
        await successMessage1.setMessageType(successMessageType);
        await  successMessage1.setMessageUser(user1);
        let commonMessage2 = await Message.create( { content: '这是一条2333普通的消息', isPublished: false, isRead: false } );
